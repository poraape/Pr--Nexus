...
export POSTGRES_DSN="${POSTGRES_DSN:-postgresql+psycopg://postgres:postgres@db:5432/nexus}"
export STORAGE_PATH="${STORAGE_PATH:-backend/storage/uploads}"
...
export CHROMA_PERSIST_DIRECTORY="${CHROMA_PERSIST_DIRECTORY:-backend/.chroma}"
...
export UVICORN_PORT="${UVICORN_PORT:-7860}"

alembic -c backend/alembic.ini upgrade head

exec uvicorn backend.main:app --host "${UVICORN_HOST}" --port "${UVICORN_PORT}"
